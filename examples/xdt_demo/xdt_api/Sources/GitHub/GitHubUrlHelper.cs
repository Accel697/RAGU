...
public static class GitHubUrlHelper
{
    ...
    public static string ConvertToApiUrl(string sourceUrl, string productUrl)
    {
        try
        {
            ...
            var apiUrl = $"{ApiBase}/repos/{owner}/{repo}/{endpoint}?per_page=5";
            
            Console.WriteLine($"[GitHub] Parsed owner='{owner}', repo='{repo}' from '{productUrl}'");
            Console.WriteLine($"[GitHub] API URL: {apiUrl}");
